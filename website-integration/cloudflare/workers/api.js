  ...
  async fetch(request, env, ctx) {
    ...
    try {
      if (path.startsWith('/api/leads')) {
        ...
        return await handleAgents(request, env, path);
...
async function handleAgents(request, env, path) {
  if (request.method === 'GET') {
    const results = await env.DB.prepare('SELECT * FROM agents WHERE active = 1 ORDER BY name').all();
    return jsonResponse({ agents: results.results });
  }
